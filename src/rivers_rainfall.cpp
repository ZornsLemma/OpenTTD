...
int32 RainfallRiverGenerator::LakePathSearch_EndNodeCheck(const AyStar *aystar, const OpenListNode *current)
{
...
void PrintRunningTimeToDebug(const char* step, std::chrono::time_point<std::chrono::system_clock> &base_time)
{
	auto old_base_time = base_time;
	base_time = std::chrono::system_clock::now();
	std::chrono::milliseconds length_ms = std::chrono::duration_cast<std::chrono::milliseconds>(base_time - old_base_time);
	int64 length = static_cast<int64>(length_ms.count());
...
bool RainfallRiverGenerator::GenerateRivers()
{
	...
	PrintRunningTimeToDebug(" (1) HeightIndex: ", base_time);
	...
	PrintRunningTimeToDebug(" (2) Removing small basins: ", base_time);
	...
	PrintRunningTimeToDebug(" (3) Number of lower tiles: ", base_time);
	...
	PrintRunningTimeToDebug(" (4) Flow: ", base_time);
	...
	PrintRunningTimeToDebug(" (5) Modify flow: ", base_time);
	...
	PrintRunningTimeToDebug(" (6) Define lakes: ", base_time);
	...
	PrintRunningTimeToDebug(" (7) Prepare rivers and lakes: ", base_time);
	...
	PrintRunningTimeToDebug(" (8) Wider rivers: ", base_time);

	/* (9) Find problem tiles, i.e. tiles whose slope is not suitable for becoming river. */
	std::set<TileIndex> problem_tiles = std::set<TileIndex>();
	GetProblemTiles(water_tiles, problem_tiles, water_info);
	PrintRunningTimeToDebug(" (9) Find problem tiles: ", base_time);
	...
	PrintRunningTimeToDebug(" (10) Fix by local terraforming: ", base_time);

	/* (11) Try to fix them by moving them, or by declaring them no water if they are not necessary for keeping the river connected. */
	this->FixByMovingProblemTiles(problem_tiles, water_flow, water_info, define_lakes_iterator);
	PrintRunningTimeToDebug(" (11) Fix by moving problem tiles: ", base_time);
	...
	PrintRunningTimeToDebug(" (12) Lower tiles until valid: ", base_time);
	...
	PrintRunningTimeToDebug(" (13) Fix bad inclined river slopes: ", base_time);
	...
	PrintRunningTimeToDebug(" (14) Derive rivers: ", base_time);
	...
	PrintRunningTimeToDebug(" (15) Connect rivers: ", base_time);

	/* (16) Get rid of rivers flowing upwards. */
	this->FixUpwardsRivers(river_map, river_iteration, id_to_river, water_flow, water_info);
	PrintRunningTimeToDebug(" (16) Fix upwards rivers: ", base_time);
	...
	PrintRunningTimeToDebug(" (17) Link rivers with ocean: ", base_time);
	...
	PrintRunningTimeToDebug(" (18) Fix lower terrain near rivers: ", base_time);
	...
	PrintRunningTimeToDebug(" (19) Another time lower tiles until valid: ", base_time);
	...
	PrintRunningTimeToDebug(" (20) Actually generate rivers in OpenTTD sense: ", base_time);
